...
async def trigger_experiment_batch_api(request: ExperimentBatchRequest):
    ...
    print(
        f"API: Submitting experiment batch '{request.batch_name}' (API ID: {api_assigned_batch_id}) to Celery."
    )
    try:
...
async def trigger_single_experiment_api(
    ...
    print(
        f"API: Submitting single experiment '{experiment_id}' (API ID: {api_assigned_task_id}) to Celery."
    )
    ...
    try:
        ...
        print(
            f"API Error submitting single experiment '{experiment_id}' to Celery: {e}"
        )
        raise HTTPException(
            status_code=500,
...
async def run_analysis_task_api(request: AnalysisTaskRequest):
    ...
    print(
        f"API: Submitting analysis '{task_name}' (API ID: {api_assigned_task_id}) to Celery."
    )
    try:
...
async def generate_report_api(request: ReportGenerationRequest):
    ...
    print(
        f"API: Submitting report generation task '{report_name_safe}' (API ID: {api_assigned_report_id}) to Celery."
    )
    try:
