...
print("DEBUG LLM_SERVICES: API_KEY_FROM_ENV raw value: "{API_KEY_FROM_ENV}'")
...
async def get_anthropic_completion(
    ...
    print(
        f"DEBUG LLM_SERVICES: Inside get_anthropic_completion - IS_CLIENT_SUCCESSFULLY_INITIALIZED: {IS_CLIENT_SUCCESSFULLY_INITIALIZED}, anthropic_client is None: {anthropic_client is None}"
    )
    ...
    try:
        ...
        if (
            ...
            if raw_text_from_llm:
                ...
                print("DEBUG LLM_SERVICES: Raw LLM text part: "{raw_text_from_llm}'")
                print(
            ...
            print(
                f"DEBUG LLM_SERVICES: LLM response content was missing or malformed. Response content: {str(response.content) if response else 'No response object'}"
            )
            raise Exception(
